...
function displayDataMeal(data){
    ...
    for(let i=0;i<data.length;i++){
        console.log(i)
        box += `
        <div class="card1 col-lg-3 col-md-6 col-sm-12 text-light my-3" data-id="${data[i].strCategory}">
        <i class="fa-solid fa-drumstick-bite"></i>
        <h2>${data[i].strCategory}</h2>
        <p class="p">${data[i].strCategoryDescription}</p>
    </div>`
    }
    document.getElementById("card1").innerHTML = box
