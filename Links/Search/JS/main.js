...
async function getName(Search){
    let meal = await fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${Search}`)
    let mealData = await meal.json()
    return mealData
}
async function startName(Search){
    let mealData = await getName(Search)
    console.log(mealData)
    displayData(mealData.meals)
}
startName()
let searchName = document.getElementById("searchName")
searchName.addEventListener("input",function(){
    console.log(searchName.value)
    startName(searchName.value)
})

//SearchLetter
async function getLetter(Search){
    let meal = await fetch(`https://www.themealdb.com/api/json/v1/1/search.php?f=${Search}`)
    let mealData = await meal.json()
    return mealData
}
async function startLetter(Search){
    let mealData = await getLetter(Search)
    console.log(mealData)
    displayData(mealData.meals)
}
startLetter()
...
searchLetter.addEventListener("input",function(){
    ...
    startLetter(searchLetter.value)
})
...
function displayData(data){
    ...
    for(let i=0;i<data.length;i++){
        console.log(i)
        box += `<div data-id="${data[i].idMeal}" class="card1 col-md-3 my-3 position-relative">
